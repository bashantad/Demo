...
class Default_ActivitiesController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->id = $this->view->id = $config->activities->id;
    ...
    public function detailAction()
    {
        ...
        if (strstr($cat, 'others')) {
            $cat = explode('::', $cat);
            $element = $eleCatModel->getReferencedElementCategory($this->id, $id, $cat[1]);
        } else {
            $element = $eleCatModel->getReferencedElementCategory($this->id, $id, $cat);
        }
            ...
            if (key_exists($title, $session->selection[$this->id])) {
                ...
                if (array_key_exists('tid', $session->selection[$this->id][$title])) {
                    if ($session->selection[$this->id][$title]['tid']) {
                        ...
                        $placesArray = explode(',', $session->userdata['activities']);
                        $key = array_search($tid, $placesArray);
                        unset($placesArray[$key]);
                        $session->userdata['activities'] = implode(',', $placesArray);
